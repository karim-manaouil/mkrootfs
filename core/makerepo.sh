...
HOST_PACKAGES="/var/cache/apt/archives/"
...
echo "deb [trusted=true] http://localhost/debian/amd64 /" > /etc/apt/sources.list

info "Starting local apt server ..."
docker run -d -p 8778:80 -v "${HOST_PACKAGES}":"${SERVER_PACKAGES}" \
